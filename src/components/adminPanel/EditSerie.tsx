...
import AddSeriesLinks from './customInputes/AddSeriesLinks';
import { SeriesInput } from '@/types';
interface PropsType {
  bottomButtons: ReactElement;
}

const EditSeries = ({ bottomButtons }: PropsType) => {
  const {
    register,
    formState: { errors },
    control,
  } = useFormContext<SeriesInput>();
  ...
  const series_urls = useWatch({ name: 'serie_urls' });
  ...
  return (
    <div className="flex flex-col gap-7">
      ...
      <AddSeriesLinks series_urls={series_urls} />
