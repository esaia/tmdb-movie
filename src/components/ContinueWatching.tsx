...
import { continueWatchingType } from '@/types';
import Link from 'next/link';
import { generateSlug } from '@/lib/helpers';

const ContinueWatching = () => {
  const [isClient, setIsClient] = useState(false);
  const [movies, setMovies] = useState<continueWatchingType[]>();
              ...
              movies.slice(0, 10).map(movie => {
                return (
                  <SwiperSlide className="relative !w-[260px] cursor-pointer pr-2 lg:!w-[440px] lg:pr-6" key={movie.id}>
                    <Link
                      href={`/${movie?.season ? 'serie' : 'movie'}/${generateSlug(movie)}`}
                    ...
                    >
                      ...
                      <div className="absolute bottom-0 left-0 flex h-20 w-full flex-col items-start justify-center bg-black/50 px-2 py-3 md:h-24 lg:h-28">
                        <p className="text line-clamp-2">{movie.title_ka}</p>
                        <p className="text line-clamp-2">{movie.title_en}</p>
                      </div>
                    </Link>
